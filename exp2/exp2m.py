...
try:
	training_portion = float(sys.argv[1])
except Exception, e:
	print('-- {}'.format(e))

X_train = X_train[:int(training_portion * X_train.shape[0])]
y_train = y_train[:int(training_portion * y_train.shape[0])]
...
try:
	filter_row = int(sys.argv[2])
	filter_col = int(sys.argv[3])
	num_core = int(sys.argv[4])
except Exception, e:
	print('-- {}'.format(e))

print('-- Filter size  is {} x {}'.format(filter_row, filter_col))
print('-- Number of core is {}'.format(num_core))
...
plt.savefig('exp2m_c{}.png'.format(num_core), format='png')
plt.close()
