...
for curRegularizationScale in regularizationScales:
	startOfLoop = time.time()
	print("==> Test with Regularization Scale %g"%(curRegularizationScale))
	[trainingAccuracies, testAccuracies, trainingCosts, testCosts, epochNumbers] = runNeuralNet(121, 100, X_train, y_train, X_test, y_test, batchSize, numEpochs, curRegularizationScale, regularizationType)
	...
	endOfLoop = time.time()
	print("Test with Regularization scale %g took: %g"%(curRegularizationScale, endOfLoop - startOfLoop))
...
print("Regularization Type: %s"%regularizationType)
print("Regularization Scales: %s"%str(regularizationScales))
...
numPlots = 1 + len(regularizationScales) # 1 fig for all the validations together, then a fig for the training of each regularization type
...
trainingPlot.set_title("Validation Error vs. Epoch Number for all Regularization Scales")
...
for i in range(len(trainingAccuracyLists)):
	curRegularizationScale = regularizationScales[i]
	...
	trainingPlot.plot(epochNumbers, testCosts, label="Validation, Scale = %g"%(curRegularizationScale), marker="o", markersize="3", ls="None")
	...
	newPlot.set_title("Error vs. Epoch Number, Scale: %g"%(curRegularizationScale))
	newPlot.plot(epochNumbers, trainingCosts, label="Training, Scale = %g"%(curRegularizationScale), marker="o", markersize="3", ls="None")
	newPlot.plot(epochNumbers, testCosts, label="Validation, Scale = %g"%(curRegularizationScale), marker="o", markersize="3", ls="None")
...
fig.tight_layout()
fig.savefig('exp1o_RegularizationScales.png')
