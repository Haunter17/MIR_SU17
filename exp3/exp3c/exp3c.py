...
trainingPlotTitles.append('Exp 3c, Validation Cross-Entropy Cost vs. Epoch')
trainingPlotFiles = ['exp3c_training_%gx%g_k1=%g_%gEpochs.png'%(filterRows[i], filterCols[i], k1s[i], numEpochs) for i in range(len(filterRows))]
trainingPlotFiles.append('exp3c_validationCurves_%gEpochs'%(numEpochs))
...
makeEndResultPlot(filterCols, train_err_lists, val_err_lists, "Width of Horizontal Kernel", "Cross Entropy Cost", 'Cost vs. Width of Horizontal Filter, k1=%g'%k1s[0], finalPlotName)
