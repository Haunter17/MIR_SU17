...
class Model:
	def __init__(self, num_freq, X_train, y_train, X_val, y_val, filter_row, filter_col, k1, num_hidden_nodes_one, learningRate, debug):
		'''
		...
		self.num_freq, self.filter_row, self.filter_col, self.k1, self.num_hidden_nodes_one, self.learningRate, self.debug = num_freq, filter_row, filter_col, k1, num_hidden_nodes_one, learningRate, debug
		...
		print("Setting up Single Conv Layer Neural net with %g x %g filters, k1 = %g, followed by one hidden layer with %g nodes, learningRate = %g"%(filter_row, filter_col, k1, num_hidden_nodes_one, learningRate))
	...
	def train(self, batch_size, num_epochs, print_freq, trainingKeepProb, debug_out='debug.txt'):
		'''
		...
		for epoch in range(num_epochs):
			...
			for i in range(0, self.num_training_vec, batch_size):
			  ...
			  self.train_step.run(feed_dict={self.x: train_batch_data, self.y_: train_batch_label, self.keep_prob:trainingKeepProb})
			epochEnd = time.time()
			...
			if (epoch + 1) % print_freq == 0:
				...
				train_acc = self.evalByBatch(self.accuracy, X_train, y_trainOH, 5000, evaluationKeepProb)
				train_acc_list.append(train_acc)
				val_acc = self.evalByBatch(self.accuracy, X_val, y_valOH, 5000, evaluationKeepProb)
				val_acc_list.append(val_acc)
				# Now we compute the error on each set:
				train_err = self.evalByBatch(self.cross_entropy, X_train, y_trainOH, 5000, evaluationKeepProb)
				train_err_list.append(train_err)
				val_err = self.evalByBatch(self.cross_entropy, X_val, y_valOH, 5000, evaluationKeepProb)
	...
	def evalByBatch(self, toEval, x, y_, batchSize, evaluationKeepProb):
		weightedAvg = 0.0
		...
		for i in range(0, len(x), batchSize):
			...
			curAmount = toEval.eval(feed_dict={self.x: batch_data, self.y_: batch_label, self.keep_prob:evaluationKeepProb})
