...
class Model:
	...
	def y_conv(self):
		...
		h_conv1 = tf.nn.relu(conv2d(x_image, self.W_conv1) + self.b_conv1) # expand dims for the 3d convolution
		...
		self.W_conv2 = init_weight_variable([self.filter_row_2, self.filter_col_2, self.k1, self.k2], "W_conv2")
		self.b_conv2 = init_bias_variable([self.k2], "b_conv_2")

		# compute the output of the second convolutional layer
		h_conv2 = tf.nn.relu(conv2d(h_conv1, self.W_conv2))
		...
		h_conv2_flat = tf.reshape(h_conv2, [-1, (conv1_height - self.filter_row_2 + 1) * (conv1_width - self.filter_col_2 + 1) *  self.k2])
		h_conv2_flat = tf.identity(h_conv2_flat, name="h_conv2_flat")
		print("shape of flat:")
		print(h_conv2_flat.get_shape())
		# softmax layer parameters
		self.W_sm = init_weight_variable([(conv1_height - self.filter_row_2 + 1) * (conv1_width - self.filter_col_2 + 1) *  self.k2, self.num_classes], "W_sm")
