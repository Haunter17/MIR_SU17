...
class Model:
	def __init__(self, num_freq, X_train, y_train, X_val, y_val, filter_row_1, filter_col_1, filter_row_2, filter_col_2, k1, k2, learningRate, debug):
		...
		print("Setting up two Conv Layer Neural net with %g x %g and %g x %g filters,  k1 = %g, k2 = %g, learningRate = %g"%(filter_row_1, filter_col_1, filter_row_2, filter_col_2, k1, k2, learningRate))
	...
	def y_conv(self):
		...
		h_conv1 = tf.expand_dims(tf.nn.relu(conv2d(x_image, self.W_conv1) + self.b_conv1), -1) # expand dims for the 3d convolution
		...
		self.W_conv2 = init_weight_variable([self.filter_row_2, self.filter_col_2, 1, 1, self.k2], "W_conv2")
		self.b_conv2 = init_bias_variable([self.k2], "b_conv_2")

		# compute the output of the second convolutional layer
		h_conv2 = tf.nn.relu(conv3d(h_conv1, self.W_conv2))
		...
		h_conv2_flat = tf.reshape(h_conv2, [-1, (conv1_height - self.filter_row_2 + 1) * (conv1_width - self.filter_col_2 + 1) * self.k1 *  self.k2])
		h_conv2_flat = tf.identity(h_conv2_flat, name="h_conv2_flat")
		print("shape of flat:")
		print(h_conv2_flat.get_shape())
		# softmax layer parameters
		self.W_sm = init_weight_variable([(conv1_height - self.filter_row_2 + 1) * (conv1_width - self.filter_col_2 + 1) * self.k1 *  self.k2, self.num_classes], "W_sm")
		...
		y_conv = tf.matmul(h_conv2_flat, self.W_sm) + self.b_sm
	...
	def train(self, batch_size, num_epochs, print_freq, debug_out='debug.txt'):
		...
		return [train_acc_list, val_acc_list, train_err_list, val_err_list, epoch_numbers, self.W_conv1.eval(), self.b_conv1.eval(), self.W_conv2.eval(), self.b_conv2.eval(), self.W_sm.eval(), self.b_sm.eval()]
...
for i in range(len(filterRowsOne)):
	...
	[train_acc_list, val_acc_list, train_err_list, val_err_list, epoch_numbers, W_conv1, b_conv1, W_conv2, b_conv2, W_sm, b_sm] = m.train(1000, numEpochs, print_freq)
	...
	W_conv2_lists.append(W_conv2.tolist())
	b_conv2_lists.append(b_conv2.tolist())
...
print("Val err list: %s"%(str(val_err_lists)))
print("Epoch number lists: %s"%(str(epoch_number_lists)))
# print("Final W_conv1: %s"%str(W_conv1_lists))
# print("Final b_conv1: %s"%str(b_conv1_lists))
# print("Final W_conv2 lists: %s"%str(W_conv2_lists))
# print("Final b_conv2 lists: %s"%str(b_conv2_lists))
# print("Final W_sm lists: %s"%str(W_sm_lists))
# print("Final b_sm lists: %s"%str(b_sm_lists))
# print best values
...
makeTrainingPlots(epoch_number_lists[0], zip(zip(filterRowsOne, filterColsOne), zip(filterRowsTwo, filterColsTwo)), train_err_lists, val_err_lists, "Shape of Kernels", "Cross Entropy Cost", trainingPlotTitles, trainingPlotFiles)
