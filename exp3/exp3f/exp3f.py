...
for i in range(len(filterRows)):
	...
	m = Model(169, X_train, y_train, X_val, y_val, filterRows[i], filterCols[i], k1s[i], numHiddenUnits, learningRates[i], False)
...
print("Final W_1 lists: %s"%str(W_1_lists))
print("Final b_1 lists: %s"%str(b_1_lists))
print("Final W_sm lists: %s"%str(W_sm_lists))
print("Final b_sm lists: %s"%str(b_sm_lists))
# plotting
trainingPlotTitles = ['Single Layer CNN + Dense Layer with %gx%g kernels and k1=%g'%(filterRows[i], filterCols[i], k1s[i]) for i in range(len(filterRows))]
trainingPlotTitles.append('Exp 3f, Validation Cross-Entropy Cost vs. Epoch')
trainingPlotFiles = ['exp3f_training_%gx%g_k1=%g_numHiddenUnits=%g_%gEpochs.png'%(filterRows[i], filterCols[i], k1s[i], numHiddenUnits, numEpochs) for i in range(len(filterRows))]
trainingPlotFiles.append(finalPlotName + 'validationCurves_%gEpochs.png'%(numEpochs))
makeTrainingPlots(epoch_number_lists[0], filterCols, train_err_lists, val_err_lists, "Width of Kernel", "Cross Entropy Cost", trainingPlotTitles, trainingPlotFiles)
makeBestResultPlot(filterCols, train_err_lists, val_err_lists, "Width of Vertical Kernel", "Cross Entropy Cost", 'Best Cost vs. Width of Filter, k1=%g'%k1s[0], finalPlotName)
