...
trainingPlotTitles.append('Exp 3d, Validation Cross-Entropy Cost vs. Epoch')
trainingPlotFiles = ['exp3d_training_%gx%g_k1=%g_%gEpochs.png'%(filterRows[i], filterCols[i], k1s[i], numEpochs) for i in range(len(filterRows))]
trainingPlotFiles.append('exp3d_validationCurves_%gEpochs'%(numEpochs))
...
# this might not make sense since can vary multiple params for the shape
# could make generic one goes up to higher d plots?
#makeEndResultPlot(zip(filterRows, filterCols), train_err_lists, val_err_lists, "Shape of Kernel", "Cross Entropy Cost", 'Cost vs. Shape of Filter, k1=%g'%k1s[0], finalPlotName)
