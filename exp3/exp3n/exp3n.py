...
class Model:
	def __init__(self, num_freq, X_train, y_train, X_val, y_val, filter_row, filter_col, k1, num_hidden_nodes_one, num_hidden_nodes_two, learningRate, debug):
		'''
		...
		self.num_freq, self.filter_row, self.filter_col, self.k1, self.num_hidden_nodes_one, self.num_hidden_nodes_two, self.learningRate, self.debug = num_freq, filter_row, filter_col, k1, num_hidden_nodes_one, num_hidden_nodes_two, learningRate, debug
		...
		print("Setting up Single Conv Layer Neural net with %g x %g filters, k1 = %g, followed by two hidden layers with %g and %g nodes, learningRate = %g"%(filter_row, filter_col, k1, num_hidden_nodes_one, num_hidden_nodes_two, learningRate))
	...
	def y_conv(self):
		...
		self.W_1 = init_weight_variable([(self.num_freq - self.filter_row + 1) * (self.num_frames - self.filter_col + 1) * self.k1, self.num_hidden_nodes_one], "W1")
		self.b_1 = init_bias_variable([self.num_hidden_nodes_one], "b_1")
		a_1 = tf.nn.relu(tf.matmul(h_conv1_flat, self.W_1) + self.b_1)

		# go through another
		self.W_2 = init_weight_variable([self.num_hidden_nodes_one, self.num_hidden_nodes_two], "W2")
		self.b_2 = init_bias_variable([self.num_hidden_nodes_two], "b_2")
		a_2 =tf.nn.relu(tf.matmul(a_1, self.W_2) + self.b_2)

		# softmax layer parameters
		self.W_sm = init_weight_variable([self.num_hidden_nodes_two, self.num_classes], "W_sm")
		...
		y_conv = tf.matmul(a_2, self.W_sm) + self.b_sm
	...
	def train(self, batch_size, num_epochs, print_freq, debug_out='debug.txt'):
		...
		return [train_acc_list, val_acc_list, train_err_list, val_err_list, epoch_numbers, self.W_conv1.eval(), self.b_conv1.eval(), self.W_1.eval(), self.b_1.eval(), self.W_sm.eval(), self.b_sm.eval()]
...
try:
	# filter dimensions and number nuber of filters
	...
	numHiddenUnitsOneString = sys.argv[4]
	numHiddenUnitsOneIn = map(int, numHiddenUnitsOneString.strip('[]').split(','))
	numHiddenUnitsTwoString = sys.argv[5]
	numHiddenUnitsTwoIn = map(int, numHiddenUnitsTwoString.strip('[]').split(','))
...
numHiddenUnitsTwo = numHiddenUnitsTwoIn
...
for i in range(len(filterRows)):
	...
	m = Model(169, X_train, y_train, X_val, y_val, filterRows[i], filterCols[i], k1s[i], numHiddenUnitsOne[i], numHiddenUnitsTwo[i], learningRates[i], False)
	# actually train the model (on the data it already loaded)
	[train_acc_list, val_acc_list, train_err_list, val_err_list, epoch_numbers, W_conv1, b_conv1, W_1, b_1, W_sm, b_sm] = m.train(1000, numEpochs, print_freq)
	...
	W_conv1_lists.append(W_conv1.tolist())
	b_conv1_lists.append(b_conv1.tolist())
	W_1_lists.append(W_1.tolist())
	b_1_lists.append(b_1.tolist())
	W_sm_lists.append(W_sm.tolist())
	b_sm_lists.append(b_sm.tolist())
	...
	print("K1s: %s"%(k1s))
	print("numHiddenUnitsOne = %s"%str(numHiddenUnitsOne))
	print("numHiddenUnitsTwo = %s"%str(numHiddenUnitsTwo))
...
# plotting
trainingPlotTitles = ['Single Layer CNN + two Dense Layers with %gx%g kernels, k1=%g, %g and %g hidden nodes'%(filterRows[i], filterCols[i], k1s[i], numHiddenUnitsOne[i], numHiddenUnitsTwo[i]) for i in range(len(filterRows))]
trainingPlotTitles.append('Exp 3n, Validation Cross-Entropy Cost vs. Epoch')
trainingPlotFiles = ['exp3n_training_%gx%g_k1=%g_numHiddenUnits=%g_and_%g_%gEpochs.png'%(filterRows[i], filterCols[i], k1s[i], numHiddenUnitsOne[i], numHiddenUnitsTwo[i], numEpochs) for i in range(len(filterRows))]
trainingPlotFiles.append(finalPlotName + 'validationCurves_%gEpochs.png'%(numEpochs))
makeTrainingPlots(epoch_number_lists[0], zip(filterRows, filterCols), train_err_lists, val_err_lists, "Shape and nodes", "Cross Entropy Cost", trainingPlotTitles, trainingPlotFiles)
