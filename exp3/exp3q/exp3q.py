...
class Model:
	def __init__(self, num_freq, X_train, y_train, X_val, y_val, filter_row, filter_col, k1, pool_rows, pool_cols, pool_step_rows, pool_step_cols, learningRate, debug):
		'''
	...
	def y_conv(self):
		...
		W_sm = init_weight_variable([h_pool1_out_rows * h_pool1_out_cols * self.k1, self.num_classes], "W_sm")
...
k1s = k1sIn
poolingRows = poolingRowsIn
poolingCols = poolingColsIn
poolingStepRows = poolingStepRowsIn
poolingStepCols = poolingStepColsIn
...
for i in range(len(filterRows)):
	...
	m = Model(169, X_train, y_train, X_val, y_val, filterRows[i], filterCols[i], k1s[i], poolingRows[i], poolingCols[i], poolingStepRows[i], poolingStepCols[i], learningRates[i], False)
...
print("K1s: %s"%(k1s))
print("poolingRows = %s"%str(poolingRows))
print("poolingCols = %s"%str(poolingCols))
print("poolingStepRows = %s"%str(poolingStepRows))
print("poolingStepCols = %s"%str(poolingStepCols))
