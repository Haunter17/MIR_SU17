...
try:
	...
	# read in the number of hidden units for the dense layer after the convolutional layer
	numHiddenUnits = int(sys.argv[3])
...
trainingPlotTitles.append('Exp 3g, Validation Cross-Entropy Cost vs. Epoch')
trainingPlotFiles = ['exp3g_training_%gx%g_k1=%g_hiddenUnits=%g_%gEpochs.png'%(filterRows[i], filterCols[i], k1s[i], numHiddenUnits, numEpochs) for i in range(len(filterRows))]
