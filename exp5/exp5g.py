...
print('==> Experiment 5g')
filepath = '/pylon2/ci560sp/mint96/exp5g_d7_1s.mat'
...
# Window evaluation with sub-window size = 4

for strideSize in [1,2]:
    ...
    for t in range(len(X_test)):
        ...
        x_image_single = np.reshape(this_x, (-1, 169))
        #print(x_image_single.shape)
        x_image = []
        for j in range(0, len(x_image_single)-sub_window_size+1, strideSize):
            x_image.append(np.reshape(x_image_single[j:j+sub_window_size], (num_features, -1)))

        x_image = np.reshape(x_image, (-1, num_features))
    ...
    print("Stride size = %d: acc %g, err: %g"%(strideSize, total_acc/len(X_test), total_error/len(X_test)))
