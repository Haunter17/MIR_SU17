...
# Convolutional Layer
x_window = tf.placeholder(tf.float32, [None, num_classes*num_frames])
x_window_image = tf.reshape(x_window, [-1, num_frames, num_classes, 1])
W_conv = weight_variable([num_frames, 1, 1, 1])
b_conv = bias_variable([1])
y_conv = tf.reshape(tf.nn.relu(conv2d(x_window_image, W_conv) + b_conv), [-1, num_classes])
...
cross_entropy_window = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=y_conv))

train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)
train_step_window = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy_window)

correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(y_, 1))
correct_prediction_window = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y_, 1))

accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
accuracy_window = tf.reduce_mean(tf.cast(correct_prediction_window, tf.float32))
...
validation_accuracy = accuracy.eval(feed_dict={x:X_val, y_: y_val})
...
# Training 2: Convolutional Layer with Vertical Kernel
...
errfig.savefig('exp5b_vconv_error.png')